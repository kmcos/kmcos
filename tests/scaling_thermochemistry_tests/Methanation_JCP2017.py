...
pt.add_parameter(name='T', value=523., adjustable=True, min=400, max=800)
pt.add_parameter(name='p_COgas', value=0.01, adjustable=True, min=1e-10, max=1.e2)
pt.add_parameter(name='p_CH4gas', value=0.01, adjustable=True, min=1e-10, max=1.e2)
pt.add_parameter(name='p_H2Ogas', value=0.01, adjustable=True, min=1e-10, max=1.e2)
pt.add_parameter(name='p_H2gas', value=0.97, adjustable=True, min=1e-10, max=1.e2)

pt.add_parameter(name='A', value='6.582*2.686*angstrom**2') #area of unit cell
pt.add_parameter(name='alpha', value='0.5') #BEP slope for diffusion

#gas phase formation energies
pt.add_parameter(name='E_COgas', value='2.53')
pt.add_parameter(name='E_H2Ogas', value='0')
pt.add_parameter(name='E_CH4gas', value='0')
pt.add_parameter(name='E_H2gas', value='0')

#scaling parameters
#MF max volcano
pt.add_parameter(name='E_C', value='1.75', adjustable=True, min=0, max=3.4)
pt.add_parameter(name='E_O', value='-1.35', adjustable=True, min=-1.8, max=1.4)

#analytical MF solution to H coverage
pt.add_parameter(name='H_cov', value='(1/(1+sqrt(exp(-beta*(GibbsGas_H2gas-2*GibbsAds_H_t)*eV))))')
...
for line in lines[1:]:
    if line.strip():
        ...
        pt.add_parameter(name='E_'+str(values[0]), value='(%.7f*E_C+%.7f*E_O+(%.7f))'%(float(values[1]),float(values[2]),float(values[3])))
        pt.add_parameter(name='f_'+str(values[0]), value=values[4])
...
pt.add_parameter(name='E_C_diff', value='0.48')
...
f_NW = pt.lattice.generate_coord('f.(-1,1,0).Rh211')

#CO ads/des processes
for site in [s, t]:
    pt.add_process(name='CO_ads_%s' % str(site)[8],
...
for site in [s, t]:
    pt.add_process(name='CO_des_%s' % str(site)[8],
...
for site,name in [(t_W,'t_W'), (t_NW,'t_NW')]:
    pt.add_process(name='H_CO_s_react_%s' % name,
...
for site,name in [(f,'f'), (f_S,'f_S')]:
    pt.add_process(name='H_CO_t_react_%s' % name,
...
for site,name in [(t_W,'t_W'), (t_NW,'t_NW')]:
    pt.add_process(name='C_OH_s_react_%s' % name,
...
for site,name in [(f,'f'), (f_S,'f_S')]:
    pt.add_process(name='C_OH_t_react_%s' % name,
...
#H-C reactions
for site in [f, s, t]:
    pt.add_process(name='H_C_%s_react' % str(site)[8],
...
for site in [f, s, t]:
    pt.add_process(name='CH_%s_dis' % str(site)[8],
...
for site,name in [(s_E,'s_E'), (t,'t'), (t_N,'t_N')]:
    pt.add_process(name='H_CH_f_react_%s' % name,
...
for site in [s, t]:
    pt.add_process(name='H_CH_%s_react' % str(site)[8],
...
for site,name in [(s_E,'s_E'), (t,'t'), (t_N,'t_N')]:
    pt.add_process(name='CH2_%s_dis_f' % name,
...
for site in [s, t]:
    pt.add_process(name='CH2_%s_dis' % str(site)[8],
...
#H-CH2 reactions
for site in [s, t]:
    pt.add_process(name='H_CH2_%s_react' % str(site)[8],
...
for site in [s, t]:
    pt.add_process(name='CH3_%s_dis' % str(site)[8],
...
#H-CH3 reactions
for site in [s, t]:
    pt.add_process(name='H_CH3_%s_react' % str(site)[8],
...
for site in [s, t]:
    pt.add_process(name='CH4_%s_ads' % str(site)[8],
...
#H-OH reaction
for site in [s, t]:
    pt.add_process(name='H_OH_%s_react' % str(site)[8],
...
for site in [s, t]:
    pt.add_process(name='H2O_%s_ads' % str(site)[8],
...
for (site1,name1), (site2,name2) in [((s,'s'), (s_S,'s_S')), ((s,'s'), (s_N,'s_N')), ((t,'t'), (t_S,'t_S')), ((t,'t'), (t_N,'t_N'))]:
    pt.add_process(name='OH_OH_%s_react_%s' % (name1,name2),
...
for (site1,name1), (site2,name2) in [((s,'s'), (s_S,'s_S')), ((s,'s'), (s_N,'s_N')), ((t,'t'), (t_S,'t_S')), ((t,'t'), (t_N,'t_N'))]:
    pt.add_process(name='H2O_O_%s_react_%s' % (name1,name2),
...
#O-H reaction
for site in [s, t]:
    pt.add_process(name='H_O_%s_react' % str(site)[8],
...
for site in [s, t]:
    pt.add_process(name='OH_%s_dis' % str(site)[8],
...
for ads in ['CO', 'O', 'OH', 'CH2', 'CH3']:
    for (site1,name1), (site2,name2) in [((s,'s'), (s_N,'s_N')), ((s_N,'s_N'), (s,'s')), ((t,'t'), (t_S,'t_S')), ((t_S,'t_S'), (t,'t')), ((s,'s'), (t, 't')), ((s,'s'), (t_N, 't_N')), 
                                         ((t, 't'), (s,'s')), ((t_N, 't_N'), (s,'s'))]:
        pt.add_process(name='%s_diff_%s_%s' % (ads,name1,name2),
...
pt.filename = 'Methanation_JCP2017.xml'
pt.save()
