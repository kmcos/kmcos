...
import os
ModelName = "CO_oxidation_Ruo2__acc_backend__do_acc_steps"
backend = 'local_smart'
...
#import kmcos.cli
#kmcos.cli.main('export '+ModelName+ '.xml '+ ' -o -b '+ backend + acceleration)  #this will export the model with the standard backend.
...
import numpy as np
import importlib
importlib.import_module("runfile_acc-backend_do-acc-steps") #can't use "import runfile_non-acc-backend" because of the dashes.
...
relativeTolerance = 0.40 #The relative tolerances have to be quite large this time due to statistical fluctuations. It would probably pass most of the time with 0.10, but 0.40 is more safe and is still adequately distinct for what comes out with this algorithm.
