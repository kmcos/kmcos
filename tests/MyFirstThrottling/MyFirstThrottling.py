...
print("line 11 of MyFirstThrottling.py", model_name)
pt = Project()
pt.set_meta(author='Thomas Danielson',
            ...
            model_dimension=2)
#Add pecies
pt.add_species(name='A1', color='white')
pt.add_species(name='B1', color='green')
pt.add_species(name='C1', color='red')
pt.add_species(name='D1', color='black')
pt.add_species(name='E1', color='purple')
pt.add_species(name='B2', color='blue')
pt.add_species(name='C2', color='orange')
pt.add_species(name='D2', color='grey')

#One layer and one site at the center
layer1 = pt.add_layer(name='Site')
layer1.sites.append(Site(name='coord', pos='0.5 0.5 1',
                         default_species='A1'))

#Square cell with a single site at the center
pt.lattice.cell = np.diag([3.825, 3.825, 2.343])

#Add parameters
pt.add_parameter(name='T', value=600, adjustable=False)
...
for i in range(len(rate_constants_dict)):
	rate_constants_string = str('pt.add_parameter(name=')  \
...
for i in range(len(pre_exponentials_dict)):
	pre_exponentials_string = str('pt.add_parameter(name=')  \
...
coord = pt.lattice.generate_coord('coord.(0,0,0).Site')
