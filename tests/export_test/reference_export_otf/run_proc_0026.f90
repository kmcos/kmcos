...
subroutine run_proc_oxygen_diffusion_cus_bridge_rig0000(cell)
...
! Disable processes
    ...
    if(can_do(co_adsorption_bridge,cell + (/ 1, 0, 0, 1/))) then
        call del_proc(co_adsorption_bridge,cell + (/ 1, 0, 0, 1/))
    end if
    if(can_do(co_desorption_bridge,cell + (/ 1, 0, 0, 1/))) then
        call del_proc(co_desorption_bridge,cell + (/ 1, 0, 0, 1/))
    end if
    if(can_do(co_desorption_cus,cell + (/ 0, 0, 0, 1/))) then
        call del_proc(co_desorption_cus,cell + (/ 0, 0, 0, 1/))
    end if
    if(can_do(co_diffusion_bridge_bridge_down,cell + (/ 1, 0, 0, 1/))) then
        call del_proc(co_diffusion_bridge_bridge_down,cell + (/ 1, 0, 0, 1/))
    end if
    if(can_do(co_diffusion_bridge_bridge_down,cell + (/ 1, 1, 0, 1/))) then
        call del_proc(co_diffusion_bridge_bridge_down,cell + (/ 1, 1, 0, 1/))
    end if
    if(can_do(co_diffusion_bridge_bridge_up,cell + (/ 1, 0, 0, 1/))) then
        call del_proc(co_diffusion_bridge_bridge_up,cell + (/ 1, 0, 0, 1/))
    end if
    if(can_do(co_diffusion_bridge_bridge_up,cell + (/ 1, -1, 0, 1/))) then
        call del_proc(co_diffusion_bridge_bridge_up,cell + (/ 1, -1, 0, 1/))
    end if
    ...
    if(can_do(oxygen_adsorption_bridge_bridge,cell + (/ 1, 0, 0, 1/))) then
        call del_proc(oxygen_adsorption_bridge_bridge,cell + (/ 1, 0, 0, 1/))
    end if
    if(can_do(oxygen_adsorption_bridge_bridge,cell + (/ 1, -1, 0, 1/))) then
        call del_proc(oxygen_adsorption_bridge_bridge,cell + (/ 1, -1, 0, 1/))
    end if
    ...
    if(can_do(oxygen_diffusion_bridge_bridge_0000,cell + (/ 1, 1, 0, 1/))) then
        call del_proc(oxygen_diffusion_bridge_bridge_0000,cell + (/ 1, 1, 0, 1/))
    end if
    if(can_do(oxygen_diffusion_bridge_bridge_0001,cell + (/ 1, -1, 0, 1/))) then
        call del_proc(oxygen_diffusion_bridge_bridge_0001,cell + (/ 1, -1, 0, 1/))
    end if
    ...
    if(can_do(reaction_oxygen_bridge_co_bridg0000,cell + (/ 1, 1, 0, 1/))) then
        call del_proc(reaction_oxygen_bridge_co_bridg0000,cell + (/ 1, 1, 0, 1/))
    end if
    if(can_do(reaction_oxygen_bridge_co_bridg0001,cell + (/ 1, -1, 0, 1/))) then
        call del_proc(reaction_oxygen_bridge_co_bridg0001,cell + (/ 1, -1, 0, 1/))
    end if
...
! Enable processes
    ...
    case(empty)
        call add_proc(oxygen_diffusion_bridge_bridge_0001, cell + (/ 1, 0, 0, 1/), gr_oxygen_diffusion_bridge_bridge_0001(cell + (/ 1, 0, 0, 0/)))
    case(oxygen)
        call add_proc(oxygen_desorption_bridge_bridge, cell + (/ 1, 0, 0, 1/), gr_oxygen_desorption_bridge_bridge(cell + (/ 1, 0, 0, 0/)))
    case(co)
        call add_proc(reaction_oxygen_bridge_co_bridg0001, cell + (/ 1, 0, 0, 1/), gr_reaction_oxygen_bridge_co_bridg0001(cell + (/ 1, 0, 0, 0/)))
    end select

    select case(get_species(cell + (/1, -1, 0, ruo2_bridge/)))
    case(empty)
        call add_proc(oxygen_diffusion_bridge_bridge_0000, cell + (/ 1, 0, 0, 1/), gr_oxygen_diffusion_bridge_bridge_0000(cell + (/ 1, 0, 0, 0/)))
    case(oxygen)
        call add_proc(oxygen_desorption_bridge_bridge, cell + (/ 1, -1, 0, 1/), gr_oxygen_desorption_bridge_bridge(cell + (/ 1, -1, 0, 0/)))
    case(co)
        call add_proc(reaction_oxygen_bridge_co_bridg0000, cell + (/ 1, 0, 0, 1/), gr_reaction_oxygen_bridge_co_bridg0000(cell + (/ 1, 0, 0, 0/)))
    end select
...
end subroutine run_proc_oxygen_diffusion_cus_bridge_rig0000

end module run_proc_0026
