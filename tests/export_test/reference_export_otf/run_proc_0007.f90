...
! Disable processes
    ...
    if(can_do(oxygen_desorption_cus_cus,cell + (/ -1, -1, 0, 1/))) then
        call del_proc(oxygen_desorption_cus_cus,cell + (/ -1, -1, 0, 1/))
    end if
...
! Enable processes
    ...
    call add_proc(co_desorption_cus, cell + (/ -1, 0, 0, 1/), gr_co_desorption_cus(cell + (/ -1, 0, 0, 0/)))
