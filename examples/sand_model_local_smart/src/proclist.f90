...
integer(kind=iint), parameter, public :: blocked = 0
integer(kind=iint), parameter, public :: drain = 1
integer(kind=iint), parameter, public :: empty = 2
integer(kind=iint), parameter, public :: grain = 3
integer(kind=iint), parameter, public :: source = 4
integer(kind=iint), public :: default_species = empty


! Process constants

integer(kind=iint), parameter, public :: diffusion_down = 1
integer(kind=iint), parameter, public :: diffusion_down_left = 2
integer(kind=iint), parameter, public :: diffusion_down_right = 3
integer(kind=iint), parameter, public :: diffusion_left = 4
integer(kind=iint), parameter, public :: diffusion_right = 5
integer(kind=iint), parameter, public :: diffusion_up_left = 6
integer(kind=iint), parameter, public :: diffusion_up_right = 7
integer(kind=iint), parameter, public :: entry = 8
integer(kind=iint), parameter, public :: exit = 9
...
integer(kind=iint), parameter, public :: nr_of_proc = 9
...
!    ``none``
!******
    ! nr_of_species = 5, spuck = 1
    real(kind=rdouble), dimension(0:4, 1:1), intent(out) :: occupation
...
!******
    ...
    if (.not. no_banner) then
        print *, "+------------------------------------------------------------+"
        print *, "|                                                            |"
        print *, "| This kMC Model 'sand_model' was written by                 |"
        print *, "|                                                            |"
...
!******
    ...
    select case(proc)
    case(diffusion_down)
        call put_grain_default_a(lsite)
        call take_grain_default_a(lsite + (/0, 1, 0, 0/))

    case(diffusion_down_left)
        call put_grain_default_a(lsite)
        call take_grain_default_a(lsite + (/1, 1, 0, 0/))

    case(diffusion_down_right)
        call put_grain_default_a(lsite + (/1, -1, 0, 0/))
        call take_grain_default_a(lsite)

    case(diffusion_left)
        call put_grain_default_a(lsite + (/0, 1, 0, 0/))
        call take_grain_default_a(lsite + (/1, 1, 0, 0/))

    case(diffusion_right)
        call put_grain_default_a(lsite + (/1, 0, 0, 0/))
        call take_grain_default_a(lsite)

    case(diffusion_up_left)
        call put_grain_default_a(lsite + (/0, 1, 0, 0/))
        call take_grain_default_a(lsite + (/1, 0, 0, 0/))

    case(diffusion_up_right)
        call put_grain_default_a(lsite + (/1, 1, 0, 0/))
        call take_grain_default_a(lsite)
...
subroutine put_blocked_default_a(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, empty, blocked)

    ! disable affected processes
    if(avail_sites(diffusion_down, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(diffusion_down, site)
    endif

    if(avail_sites(diffusion_down_left, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(diffusion_down_left, site)
    endif

    if(avail_sites(diffusion_down_right, lattice2nr(site(1) + (-1), site(2) + (1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(diffusion_down_right, site + (/-1, 1, 0, 0/))
    endif

    if(avail_sites(diffusion_left, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(diffusion_left, site + (/0, -1, 0, 0/))
    endif

    if(avail_sites(diffusion_right, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(diffusion_right, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(diffusion_up_left, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(diffusion_up_left, site + (/0, -1, 0, 0/))
    endif

    if(avail_sites(diffusion_up_right, lattice2nr(site(1) + (-1), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(diffusion_up_right, site + (/-1, -1, 0, 0/))
    endif

    if(avail_sites(entry, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(entry, site)
    endif


end subroutine put_blocked_default_a

subroutine take_blocked_default_a(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, blocked, empty)
    ...
    case(grain)
        ...
        case(grain)
            call add_proc(diffusion_left, site + (/0, -1, 0, 0/))
        end select

        select case(get_species(site + (/-1, 0, 0, 0/)))
        case(grain)
            call add_proc(diffusion_right, site + (/-1, 0, 0, 0/))
        end select

        select case(get_species(site + (/1, -1, 0, 0/)))
        case(grain)
            call add_proc(diffusion_up_left, site + (/0, -1, 0, 0/))
        end select

        select case(get_species(site + (/-1, -1, 0, 0/)))
        case(grain)
            call add_proc(diffusion_up_right, site + (/-1, -1, 0, 0/))
        end select
    ...
    select case(get_species(site + (/1, 1, 0, 0/)))
    case(grain)
        call add_proc(diffusion_down_left, site)
    end select

    select case(get_species(site + (/-1, 1, 0, 0/)))
    case(grain)
        call add_proc(diffusion_down_right, site + (/-1, 1, 0, 0/))
    end select


end subroutine take_blocked_default_a

subroutine put_drain_default_a(site)
    ...
    select case(get_species(site + (/0, 1, 0, 0/)))
    case(grain)
        call add_proc(exit, site)
    end select


end subroutine put_drain_default_a

subroutine take_drain_default_a(site)
    ...
    ! disable affected processes
    if(avail_sites(exit, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(exit, site)
    endif
...
end subroutine take_drain_default_a

subroutine put_grain_default_a(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, empty, grain)
    ...
    case(empty)
        ...
        select case(get_species(site + (/-1, 1, 0, 0/)))
        case(grain)
            call add_proc(diffusion_right, site + (/-1, 1, 0, 0/))
        end select

        select case(get_species(site + (/1, 0, 0, 0/)))
        case(grain)
            call add_proc(diffusion_up_left, site)
        end select

        select case(get_species(site + (/-1, 0, 0, 0/)))
        case(grain)
            call add_proc(diffusion_up_right, site + (/-1, 0, 0, 0/))
        end select
    ...
    select case(get_species(site + (/0, -1, 0, 0/)))
    case(empty)
        call add_proc(diffusion_down, site + (/0, -1, 0, 0/))
    case(drain)
        call add_proc(exit, site + (/0, -1, 0, 0/))
    end select
    ...
    case(grain)
        select case(get_species(site + (/-1, 0, 0, 0/)))
        case(empty)
            call add_proc(diffusion_left, site + (/-1, -1, 0, 0/))
        end select

    case(empty)
        call add_proc(diffusion_down_left, site + (/-1, -1, 0, 0/))
    end select
    ...
    case(empty)
        call add_proc(diffusion_down_right, site)
    end select
    ...
    case(grain)
        select case(get_species(site + (/-1, 1, 0, 0/)))
        case(empty)
            call add_proc(diffusion_up_left, site + (/-1, 0, 0, 0/))
        end select
    ...
    case(grain)
        select case(get_species(site + (/1, 1, 0, 0/)))
        case(empty)
            call add_proc(diffusion_up_right, site)
        end select

    end select


end subroutine put_grain_default_a

subroutine take_grain_default_a(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, grain, empty)

    ! disable affected processes
    if(avail_sites(diffusion_down, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(diffusion_down, site + (/0, -1, 0, 0/))
    endif

    if(avail_sites(diffusion_down_left, lattice2nr(site(1) + (-1), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(diffusion_down_left, site + (/-1, -1, 0, 0/))
    endif

    if(avail_sites(diffusion_down_right, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(diffusion_down_right, site)
    endif

    if(avail_sites(diffusion_left, lattice2nr(site(1) + (-1), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(diffusion_left, site + (/-1, -1, 0, 0/))
    endif

    if(avail_sites(diffusion_left, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(diffusion_left, site)
    endif

    if(avail_sites(diffusion_right, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(diffusion_right, site)
    endif

    if(avail_sites(diffusion_right, lattice2nr(site(1) + (-1), site(2) + (1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(diffusion_right, site + (/-1, 1, 0, 0/))
    endif

    if(avail_sites(diffusion_up_left, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(diffusion_up_left, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(diffusion_up_left, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(diffusion_up_left, site)
    endif

    if(avail_sites(diffusion_up_right, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(diffusion_up_right, site)
    endif

    if(avail_sites(diffusion_up_right, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(diffusion_up_right, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(exit, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(exit, site + (/0, -1, 0, 0/))
    endif
...
end subroutine take_grain_default_a

subroutine put_source_default_a(site)
    ...
    select case(get_species(site + (/0, -1, 0, 0/)))
    case(empty)
        call add_proc(entry, site + (/0, -1, 0, 0/))
    end select


end subroutine put_source_default_a

subroutine take_source_default_a(site)
    ...
    ! disable affected processes
    if(avail_sites(entry, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(entry, site + (/0, -1, 0, 0/))
    endif
...
end subroutine take_source_default_a

subroutine touchup_default_a(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    if (can_do(diffusion_down, site)) then
        call del_proc(diffusion_down, site)
    endif
    if (can_do(diffusion_down_left, site)) then
        call del_proc(diffusion_down_left, site)
    endif
    if (can_do(diffusion_down_right, site)) then
        call del_proc(diffusion_down_right, site)
    endif
    if (can_do(diffusion_left, site)) then
        call del_proc(diffusion_left, site)
    endif
    if (can_do(diffusion_right, site)) then
        call del_proc(diffusion_right, site)
    endif
    if (can_do(diffusion_up_left, site)) then
        call del_proc(diffusion_up_left, site)
    endif
    if (can_do(diffusion_up_right, site)) then
        call del_proc(diffusion_up_right, site)
    endif
    if (can_do(entry, site)) then
        call del_proc(entry, site)
    endif
    if (can_do(exit, site)) then
        call del_proc(exit, site)
    endif
