...
!  The model was written by Christa Cody.
...
integer(kind=iint), parameter, public :: CO_desorption = 1
integer(kind=iint), parameter, public :: Do_nothing = 2
...
!******
    ...
    if (.not. no_banner) then
        print *, "+------------------------------------------------------------+"
        print *, "|                                                            |"
        print *, "| This kMC Model 'MyFirstTPD_DesorptionOnly_Comparison' was written by|"
        print *, "|                                                            |"
        print *, "|               Christa Cody (codycn@ornl.gov)               |"
        print *, "|                                                            |"
    ...
    do k = 0, system_size(3)-1
        do j = 0, system_size(2)-1
            do i = 0, system_size(1)-1
                ...
                select case(layer)
                case (simple_cubic)
                    call replace_species((/i, j, k, simple_cubic_coord1/), null_species, empty)
                end select
            end do
        end do
    end do
    ...
    do k = 0, system_size(3)-1
        do j = 0, system_size(2)-1
            do i = 0, system_size(1)-1
                select case(layer)
                case(simple_cubic)
                    call touchup_simple_cubic_coord1((/i, j, k, simple_cubic_coord1/))
                end select
            end do
        end do
    end do
...
!******
    ...
    select case(proc)
    case(CO_desorption)
        call take_CO_simple_cubic_coord1(lsite)
...
subroutine put_CO_simple_cubic_coord1(site)
    ...
    call add_proc(Do_nothing, site)

end subroutine put_CO_simple_cubic_coord1

subroutine take_CO_simple_cubic_coord1(site)
    ...
    if(avail_sites(Do_nothing, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(Do_nothing, site)
    endif


end subroutine take_CO_simple_cubic_coord1

subroutine touchup_simple_cubic_coord1(site)
    ...
    if (can_do(Do_nothing, site)) then
        call del_proc(Do_nothing, site)
    endif
...
end subroutine touchup_simple_cubic_coord1
