...
!  The model was written by StangenMensch.
...
integer(kind=iint), parameter, public :: des = 2
integer(kind=iint), parameter, public :: left = 3
integer(kind=iint), parameter, public :: right = 4


integer(kind=iint), parameter, public :: representation_length = 20
...
!******
    ...
    if (.not. no_banner) then
        print *, "+------------------------------------------------------------+"
        print *, "|                                                            |"
        print *, "| This kMC Model 'hopping_model' was written by              |"
        print *, "|                                                            |"
        print *, "|                StangenMensch (linie@tum.de)                |"
        print *, "|                                                            |"
...
!******
    ...
    select case(proc)
    case(ads)
        call put_C_default_a(lsite)

    case(des)
        call take_C_default_a(lsite)

    case(left)
        call take_C_default_a(lsite + (/1, 0, 0, 0/))
        call put_C_default_a(lsite)

    case(right)
        call take_C_default_a(lsite)
        call put_C_default_a(lsite + (/1, 0, 0, 0/))

    end select

end subroutine run_proc_nr

subroutine put_C_default_a(site)
    ...
    if(avail_sites(left, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(left, site)
    endif

    if(avail_sites(right, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(right, site + (/-1, 0, 0, 0/))
    endif

    ! enable affected processes
    call add_proc(des, site)
    select case(get_species(site + (/-1, 0, 0, 0/)))
    case(empty)
        call add_proc(left, site + (/-1, 0, 0, 0/))
    end select

    select case(get_species(site + (/1, 0, 0, 0/)))
    case(empty)
        call add_proc(right, site)
    end select


end subroutine put_C_default_a

subroutine take_C_default_a(site)
    ...
    ! disable affected processes
    if(avail_sites(des, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(des, site)
    endif

    if(avail_sites(left, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(left, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(right, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(right, site)
    endif
    ...
    select case(get_species(site + (/1, 0, 0, 0/)))
    case(C)
        call add_proc(left, site)
    end select

    select case(get_species(site + (/-1, 0, 0, 0/)))
    case(C)
        call add_proc(right, site + (/-1, 0, 0, 0/))
    end select


end subroutine take_C_default_a
...
subroutine touchup_default_a(site)
    ...
    if (can_do(des, site)) then
        call del_proc(des, site)
    endif
    if (can_do(left, site)) then
        call del_proc(left, site)
    endif
    if (can_do(right, site)) then
        call del_proc(right, site)
    endif
