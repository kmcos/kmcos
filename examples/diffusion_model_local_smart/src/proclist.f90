...
integer(kind=iint), parameter, public :: ion = 3
...
integer(kind=iint), parameter, public :: diffusion_left = 2
integer(kind=iint), parameter, public :: diffusion_right = 3
integer(kind=iint), parameter, public :: diffusion_up = 4
integer(kind=iint), parameter, public :: entry = 5
integer(kind=iint), parameter, public :: exit = 6
...
!******
    ...
    if (.not. no_banner) then
        print *, "+------------------------------------------------------------+"
        print *, "|                                                            |"
        print *, "| This kMC Model 'ion_diffusion_model' was written by        |"
        print *, "|                                                            |"
...
!******
    ...
    select case(proc)
    case(diffusion_down)
        call put_ion_default_a(lsite)
        call take_ion_default_a(lsite + (/0, 1, 0, 0/))

    case(diffusion_left)
        call put_ion_default_a(lsite)
        call take_ion_default_a(lsite + (/1, 0, 0, 0/))

    case(diffusion_right)
        call put_ion_default_a(lsite + (/1, 0, 0, 0/))
        call take_ion_default_a(lsite)

    case(diffusion_up)
        call put_ion_default_a(lsite + (/0, 1, 0, 0/))
        call take_ion_default_a(lsite)
...
subroutine put_blocked_default_a(site)
    ...
    if(avail_sites(diffusion_up, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(diffusion_up, site + (/0, -1, 0, 0/))
    endif
...
subroutine take_blocked_default_a(site)
    ...
    case(ion)
        call add_proc(diffusion_up, site + (/0, -1, 0, 0/))
    end select
...
subroutine put_ion_default_a(site)
    ...
    select case(get_species(site + (/-1, 0, 0, 0/)))
    case(empty)
        call add_proc(diffusion_left, site + (/-1, 0, 0, 0/))
    end select
...
end subroutine put_ion_default_a

subroutine take_ion_default_a(site)
    ...
    if(avail_sites(diffusion_left, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(diffusion_left, site + (/-1, 0, 0, 0/))
    endif
    ...
    if(avail_sites(diffusion_up, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
...
end subroutine take_ion_default_a
...
subroutine touchup_default_a(site)
    ...
    if (can_do(diffusion_up, site)) then
        call del_proc(diffusion_up, site)
    endif
