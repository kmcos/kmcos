...
integer(kind=iint), parameter, public :: H = 0
...
integer(kind=iint), parameter, public :: H_adsorption_hollow1 = 1
integer(kind=iint), parameter, public :: H_adsorption_hollow2 = 2
integer(kind=iint), parameter, public :: H_desorption_hollow1 = 3
integer(kind=iint), parameter, public :: H_desorption_hollow2 = 4
integer(kind=iint), parameter, public :: H_diff_h1h2 = 5
integer(kind=iint), parameter, public :: H_diff_h2h1 = 6
...
!    ``none``
!******
    ! nr_of_species = 2, spuck = 2
    real(kind=rdouble), dimension(0:1, 1:2), intent(out) :: occupation
...
!******
    ...
    if (.not. no_banner) then
        print *, "+------------------------------------------------------------+"
        print *, "|                                                            |"
        print *, "| This kMC Model 'pt111' was written by                      |"
        print *, "|                                                            |"
    ...
    do k = 0, system_size(3)-1
        do j = 0, system_size(2)-1
            do i = 0, system_size(1)-1
                ...
                select case(layer)
                case (pt111)
                    call replace_species((/i, j, k, pt111_hollow1/), null_species, default_species)
                    call replace_species((/i, j, k, pt111_hollow2/), null_species, default_species)
                end select
            end do
        end do
    end do
    ...
    do k = 0, system_size(3)-1
        do j = 0, system_size(2)-1
            do i = 0, system_size(1)-1
                select case(layer)
                case(pt111)
                    call touchup_pt111_hollow1((/i, j, k, pt111_hollow1/))
                    call touchup_pt111_hollow2((/i, j, k, pt111_hollow2/))
                end select
            end do
        end do
    end do
...
!******
    ...
    select case(proc)
    case(H_adsorption_hollow1)
        call put_H_pt111_hollow1(lsite)

    case(H_adsorption_hollow2)
        call put_H_pt111_hollow2(lsite)

    case(H_desorption_hollow1)
        call take_H_pt111_hollow1(lsite)

    case(H_desorption_hollow2)
        call take_H_pt111_hollow2(lsite)

    case(H_diff_h1h2)
        call take_H_pt111_hollow1(lsite)
        call put_H_pt111_hollow2(lsite + (/0, 0, 0, pt111_hollow2 - pt111_hollow1/))

    case(H_diff_h2h1)
        call take_H_pt111_hollow2(lsite + (/0, 0, 0, pt111_hollow2 - pt111_hollow1/))
        call put_H_pt111_hollow1(lsite)
...
subroutine put_H_pt111_hollow1(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, empty, H)

    ! disable affected processes
    if(avail_sites(H_adsorption_hollow1, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(H_adsorption_hollow1, site)
    endif

    if(avail_sites(H_diff_h2h1, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(H_diff_h2h1, site)
    endif

    ! enable affected processes
    call add_proc(H_desorption_hollow1, site)
    select case(get_species(site + (/0, 0, 0, pt111_hollow2 - pt111_hollow1/)))
    case(empty)
        call add_proc(H_diff_h1h2, site)
    end select


end subroutine put_H_pt111_hollow1

subroutine take_H_pt111_hollow1(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, H, empty)

    ! disable affected processes
    if(avail_sites(H_desorption_hollow1, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(H_desorption_hollow1, site)
    endif

    if(avail_sites(H_diff_h1h2, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(H_diff_h1h2, site)
    endif

    ! enable affected processes
    call add_proc(H_adsorption_hollow1, site)
    select case(get_species(site + (/0, 0, 0, pt111_hollow2 - pt111_hollow1/)))
    case(H)
        call add_proc(H_diff_h2h1, site)
    end select


end subroutine take_H_pt111_hollow1

subroutine put_H_pt111_hollow2(site)
    ...
    ! disable affected processes
    if(avail_sites(H_adsorption_hollow2, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(H_adsorption_hollow2, site)
    endif

    if(avail_sites(H_diff_h1h2, lattice2nr(site(1) + (0), site(2) + (0), site(3) + (0), site(4) + (pt111_hollow1 - pt111_hollow2)), 2).ne.0)then
        call del_proc(H_diff_h1h2, site + (/0, 0, 0, pt111_hollow1 - pt111_hollow2/))
    endif

    ! enable affected processes
    call add_proc(H_desorption_hollow2, site)
    select case(get_species(site + (/0, 0, 0, pt111_hollow1 - pt111_hollow2/)))
    case(empty)
        call add_proc(H_diff_h2h1, site + (/0, 0, 0, pt111_hollow1 - pt111_hollow2/))
    end select


end subroutine put_H_pt111_hollow2

subroutine take_H_pt111_hollow2(site)
    ...
    ! disable affected processes
    if(avail_sites(H_desorption_hollow2, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(H_desorption_hollow2, site)
    endif

    if(avail_sites(H_diff_h2h1, lattice2nr(site(1) + (0), site(2) + (0), site(3) + (0), site(4) + (pt111_hollow1 - pt111_hollow2)), 2).ne.0)then
        call del_proc(H_diff_h2h1, site + (/0, 0, 0, pt111_hollow1 - pt111_hollow2/))
    endif

    ! enable affected processes
    call add_proc(H_adsorption_hollow2, site)
    select case(get_species(site + (/0, 0, 0, pt111_hollow1 - pt111_hollow2/)))
    case(H)
        call add_proc(H_diff_h1h2, site + (/0, 0, 0, pt111_hollow1 - pt111_hollow2/))
    end select


end subroutine take_H_pt111_hollow2

subroutine touchup_pt111_hollow1(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    if (can_do(H_adsorption_hollow1, site)) then
        call del_proc(H_adsorption_hollow1, site)
    endif
    if (can_do(H_adsorption_hollow2, site)) then
        call del_proc(H_adsorption_hollow2, site)
    endif
    if (can_do(H_desorption_hollow1, site)) then
        call del_proc(H_desorption_hollow1, site)
    endif
    if (can_do(H_desorption_hollow2, site)) then
        call del_proc(H_desorption_hollow2, site)
    endif
    if (can_do(H_diff_h1h2, site)) then
        call del_proc(H_diff_h1h2, site)
    endif
    if (can_do(H_diff_h2h1, site)) then
        call del_proc(H_diff_h2h1, site)
    endif
...
end subroutine touchup_pt111_hollow1

subroutine touchup_pt111_hollow2(site)
...
end subroutine touchup_pt111_hollow2
