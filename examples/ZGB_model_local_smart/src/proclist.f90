...
!  The model was written by Ziff,Gulari,Barshad.
...
integer(kind=iint), parameter, public :: CO_oxidation1 = 3
integer(kind=iint), parameter, public :: CO_oxidation2 = 4
integer(kind=iint), parameter, public :: CO_oxidation3 = 5
integer(kind=iint), parameter, public :: CO_oxidation4 = 6
integer(kind=iint), parameter, public :: O2_adsorption1 = 7
integer(kind=iint), parameter, public :: O2_adsorption2 = 8
integer(kind=iint), parameter, public :: O2_desorption1 = 9
integer(kind=iint), parameter, public :: O2_desorption2 = 10
...
!******
    ...
    if (.not. no_banner) then
        print *, "+------------------------------------------------------------+"
        print *, "|                                                            |"
        print *, "| This kMC Model 'zgb_model' was written by                  |"
        print *, "|                                                            |"
        print *, "|         Ziff,Gulari,Barshad (mjhoffmann@gmail.com)         |"
        print *, "|                                                            |"
...
!******
    ...
    select case(proc)
    case(CO_adsorption)
        call put_CO_sc_site(lsite)

    case(CO_desorption)
        call take_CO_sc_site(lsite)

    case(CO_oxidation1)
        call take_CO_sc_site(lsite)
        call take_O_sc_site(lsite + (/1, 0, 0, 0/))

    case(CO_oxidation2)
        call take_CO_sc_site(lsite)
        call take_O_sc_site(lsite + (/0, 1, 0, 0/))

    case(CO_oxidation3)
        call take_CO_sc_site(lsite + (/1, 0, 0, 0/))
        call take_O_sc_site(lsite)

    case(CO_oxidation4)
        call take_CO_sc_site(lsite + (/0, 1, 0, 0/))
        call take_O_sc_site(lsite)

    case(O2_adsorption1)
        call put_O_sc_site(lsite)
        call put_O_sc_site(lsite + (/1, 0, 0, 0/))

    case(O2_adsorption2)
        call put_O_sc_site(lsite)
        call put_O_sc_site(lsite + (/0, 1, 0, 0/))
...
subroutine put_CO_sc_site(site)
    ...
    if(avail_sites(O2_adsorption1, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(O2_adsorption1, site)
    endif

    if(avail_sites(O2_adsorption1, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(O2_adsorption1, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(O2_adsorption2, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(O2_adsorption2, site)
    endif

    if(avail_sites(O2_adsorption2, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(O2_adsorption2, site + (/0, -1, 0, 0/))
    endif
    ...
    case(O)
        call add_proc(CO_oxidation1, site)
    end select

    select case(get_species(site + (/0, 1, 0, 0/)))
    case(O)
        call add_proc(CO_oxidation2, site)
    end select

    select case(get_species(site + (/-1, 0, 0, 0/)))
    case(O)
        call add_proc(CO_oxidation3, site + (/-1, 0, 0, 0/))
    end select

    select case(get_species(site + (/0, -1, 0, 0/)))
    case(O)
        call add_proc(CO_oxidation4, site + (/0, -1, 0, 0/))
    end select


end subroutine put_CO_sc_site

subroutine take_CO_sc_site(site)
    ...
    if(avail_sites(CO_oxidation1, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(CO_oxidation1, site)
    endif

    if(avail_sites(CO_oxidation2, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(CO_oxidation2, site)
    endif

    if(avail_sites(CO_oxidation3, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(CO_oxidation3, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(CO_oxidation4, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(CO_oxidation4, site + (/0, -1, 0, 0/))
    endif
    ...
    select case(get_species(site + (/1, 0, 0, 0/)))
    case(empty)
        call add_proc(O2_adsorption1, site)
    end select

    select case(get_species(site + (/-1, 0, 0, 0/)))
    case(empty)
        call add_proc(O2_adsorption1, site + (/-1, 0, 0, 0/))
    end select

    select case(get_species(site + (/0, 1, 0, 0/)))
    case(empty)
        call add_proc(O2_adsorption2, site)
    end select

    select case(get_species(site + (/0, -1, 0, 0/)))
    case(empty)
        call add_proc(O2_adsorption2, site + (/0, -1, 0, 0/))
    end select


end subroutine take_CO_sc_site

subroutine put_O_sc_site(site)
    ...
    select case(get_species(site + (/-1, 0, 0, 0/)))
    case(O)
        call add_proc(O2_desorption1, site + (/-1, 0, 0, 0/))
    case(CO)
        call add_proc(CO_oxidation1, site + (/-1, 0, 0, 0/))
    end select

    select case(get_species(site + (/0, -1, 0, 0/)))
    case(O)
        call add_proc(O2_desorption2, site + (/0, -1, 0, 0/))
    case(CO)
        call add_proc(CO_oxidation2, site + (/0, -1, 0, 0/))
    end select

    select case(get_species(site + (/1, 0, 0, 0/)))
    case(O)
        call add_proc(O2_desorption1, site)
    case(CO)
        call add_proc(CO_oxidation3, site)
    end select

    select case(get_species(site + (/0, 1, 0, 0/)))
    case(O)
        call add_proc(O2_desorption2, site)
    case(CO)
        call add_proc(CO_oxidation4, site)
    end select


end subroutine put_O_sc_site

subroutine take_O_sc_site(site)
    ...
    ! disable affected processes
    if(avail_sites(CO_oxidation1, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(CO_oxidation1, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(CO_oxidation2, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(CO_oxidation2, site + (/0, -1, 0, 0/))
    endif

    if(avail_sites(CO_oxidation3, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(CO_oxidation3, site)
    endif

    if(avail_sites(CO_oxidation4, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(CO_oxidation4, site)
    endif

    if(avail_sites(O2_desorption1, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(O2_desorption1, site)
    endif

    if(avail_sites(O2_desorption1, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(O2_desorption1, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(O2_desorption2, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(O2_desorption2, site)
    endif

    if(avail_sites(O2_desorption2, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(O2_desorption2, site + (/0, -1, 0, 0/))
    endif
...
subroutine touchup_sc_site(site)
    ...
    if (can_do(CO_oxidation1, site)) then
        call del_proc(CO_oxidation1, site)
    endif
    if (can_do(CO_oxidation2, site)) then
        call del_proc(CO_oxidation2, site)
    endif
    if (can_do(CO_oxidation3, site)) then
        call del_proc(CO_oxidation3, site)
    endif
    if (can_do(CO_oxidation4, site)) then
        call del_proc(CO_oxidation4, site)
    endif
    if (can_do(O2_adsorption1, site)) then
        call del_proc(O2_adsorption1, site)
    endif
    if (can_do(O2_adsorption2, site)) then
        call del_proc(O2_adsorption2, site)
    endif
    if (can_do(O2_desorption1, site)) then
        call del_proc(O2_desorption1, site)
    endif
    if (can_do(O2_desorption2, site)) then
        call del_proc(O2_desorption2, site)
    endif
