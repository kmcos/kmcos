...
!  The model was written by LotkaVolterra.
...
integer(kind=iint), parameter, public :: AA_creation1 = 1
integer(kind=iint), parameter, public :: AA_creation2 = 2
integer(kind=iint), parameter, public :: AA_creation3 = 3
integer(kind=iint), parameter, public :: AA_creation4 = 4
integer(kind=iint), parameter, public :: AB_reaction1 = 5
integer(kind=iint), parameter, public :: AB_reaction2 = 6
integer(kind=iint), parameter, public :: AB_reaction3 = 7
integer(kind=iint), parameter, public :: AB_reaction4 = 8
integer(kind=iint), parameter, public :: B_desorption = 9


integer(kind=iint), parameter, public :: representation_length = 10
...
!******
    ...
    if (.not. no_banner) then
        print *, "+------------------------------------------------------------+"
        print *, "|                                                            |"
        print *, "| This kMC Model 'lotka_volterra_model' was written by       |"
        print *, "|                                                            |"
        print *, "|            LotkaVolterra (mjhoffmann@gmail.com)            |"
        print *, "|                                                            |"
    ...
    do k = 0, system_size(3)-1
        do j = 0, system_size(2)-1
            do i = 0, system_size(1)-1
                ...
                select case(layer)
                case (sc)
                    call replace_species((/i, j, k, sc_site/), null_species, default_species)
                end select
            end do
        end do
    end do
    ...
    do k = 0, system_size(3)-1
        do j = 0, system_size(2)-1
            do i = 0, system_size(1)-1
                select case(layer)
                case(sc)
                    call touchup_sc_site((/i, j, k, sc_site/))
                end select
            end do
        end do
    end do
...
!******
    ...
    select case(proc)
    case(AA_creation1)
        call put_A_sc_site(lsite + (/1, 0, 0, 0/))

    case(AA_creation2)
        call put_A_sc_site(lsite)

    case(AA_creation3)
        call put_A_sc_site(lsite + (/0, 1, 0, 0/))

    case(AA_creation4)
        call put_A_sc_site(lsite)

    case(AB_reaction1)
        call take_A_sc_site(lsite)
        call put_B_sc_site(lsite)

    case(AB_reaction2)
        call take_A_sc_site(lsite + (/1, 0, 0, 0/))
        call put_B_sc_site(lsite + (/1, 0, 0, 0/))
    ...
    case(AB_reaction4)
        call take_A_sc_site(lsite + (/0, 1, 0, 0/))
        call put_B_sc_site(lsite + (/0, 1, 0, 0/))

    case(B_desorption)
        call take_B_sc_site(lsite)

    end select

end subroutine run_proc_nr

subroutine put_A_sc_site(site)
    ...
    if(avail_sites(AA_creation1, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(AA_creation1, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(AA_creation2, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(AA_creation2, site)
    endif

    if(avail_sites(AA_creation3, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(AA_creation3, site + (/0, -1, 0, 0/))
    endif

    if(avail_sites(AA_creation4, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(AA_creation4, site)
    endif

    ! enable affected processes
    select case(get_species(site + (/1, 0, 0, 0/)))
    case(empty)
        call add_proc(AA_creation1, site)
    case(B)
        call add_proc(AB_reaction1, site)
    end select

    select case(get_species(site + (/-1, 0, 0, 0/)))
    case(empty)
        call add_proc(AA_creation2, site + (/-1, 0, 0, 0/))
    case(B)
        call add_proc(AB_reaction2, site + (/-1, 0, 0, 0/))
    end select

    select case(get_species(site + (/0, 1, 0, 0/)))
    case(empty)
        call add_proc(AA_creation3, site)
    case(B)
        call add_proc(AB_reaction3, site)
    end select

    select case(get_species(site + (/0, -1, 0, 0/)))
    case(empty)
        call add_proc(AA_creation4, site + (/0, -1, 0, 0/))
    case(B)
        call add_proc(AB_reaction4, site + (/0, -1, 0, 0/))
    end select


end subroutine put_A_sc_site

subroutine take_A_sc_site(site)
    ...
    ! disable affected processes
    if(avail_sites(AA_creation1, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(AA_creation1, site)
    endif

    if(avail_sites(AA_creation2, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(AA_creation2, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(AA_creation3, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(AA_creation3, site)
    endif

    if(avail_sites(AA_creation4, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(AA_creation4, site + (/0, -1, 0, 0/))
    endif

    if(avail_sites(AB_reaction1, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(AB_reaction1, site)
    endif

    if(avail_sites(AB_reaction2, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(AB_reaction2, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(AB_reaction3, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(AB_reaction3, site)
    endif

    if(avail_sites(AB_reaction4, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(AB_reaction4, site + (/0, -1, 0, 0/))
    endif

    ! enable affected processes
    select case(get_species(site + (/-1, 0, 0, 0/)))
    case(A)
        call add_proc(AA_creation1, site + (/-1, 0, 0, 0/))
    end select

    select case(get_species(site + (/1, 0, 0, 0/)))
    case(A)
        call add_proc(AA_creation2, site)
    end select

    select case(get_species(site + (/0, -1, 0, 0/)))
    case(A)
        call add_proc(AA_creation3, site + (/0, -1, 0, 0/))
    end select

    select case(get_species(site + (/0, 1, 0, 0/)))
    case(A)
        call add_proc(AA_creation4, site)
    end select


end subroutine take_A_sc_site

subroutine put_B_sc_site(site)
    ...
    select case(get_species(site + (/-1, 0, 0, 0/)))
    case(A)
        call add_proc(AB_reaction1, site + (/-1, 0, 0, 0/))
    end select

    select case(get_species(site + (/1, 0, 0, 0/)))
    case(A)
        call add_proc(AB_reaction2, site)
    end select

    select case(get_species(site + (/0, -1, 0, 0/)))
    case(A)
        call add_proc(AB_reaction3, site + (/0, -1, 0, 0/))
    end select

    select case(get_species(site + (/0, 1, 0, 0/)))
    case(A)
        call add_proc(AB_reaction4, site)
    end select


end subroutine put_B_sc_site

subroutine take_B_sc_site(site)
    ...
    ! disable affected processes
    if(avail_sites(AB_reaction1, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(AB_reaction1, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(AB_reaction2, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(AB_reaction2, site)
    endif

    if(avail_sites(AB_reaction3, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(AB_reaction3, site + (/0, -1, 0, 0/))
    endif

    if(avail_sites(AB_reaction4, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(AB_reaction4, site)
    endif
...
end subroutine take_B_sc_site

subroutine touchup_sc_site(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    if (can_do(AA_creation1, site)) then
        call del_proc(AA_creation1, site)
    endif
    if (can_do(AA_creation2, site)) then
        call del_proc(AA_creation2, site)
    endif
    if (can_do(AA_creation3, site)) then
        call del_proc(AA_creation3, site)
    endif
    if (can_do(AA_creation4, site)) then
        call del_proc(AA_creation4, site)
    endif
    if (can_do(AB_reaction1, site)) then
        call del_proc(AB_reaction1, site)
    endif
    if (can_do(AB_reaction2, site)) then
        call del_proc(AB_reaction2, site)
    endif
    if (can_do(AB_reaction3, site)) then
        call del_proc(AB_reaction3, site)
    endif
    if (can_do(AB_reaction4, site)) then
        call del_proc(AB_reaction4, site)
    endif
...
end subroutine touchup_sc_site
