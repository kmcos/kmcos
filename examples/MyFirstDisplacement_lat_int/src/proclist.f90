...
subroutine run_proc_nr(proc, nr_cell)
    ...
    select case(proc)
    case(exc_left_down)
        call run_proc_exc_left_down(cell)
    case(exc_left_up)
        call run_proc_exc_left_up(cell)
    case(exc_right_down)
        call run_proc_exc_right_down(cell)
    case(exc_right_up)
        call run_proc_exc_right_up(cell)
    case(hop_down)
        call run_proc_hop_down(cell)
    case(hop_left)
        call run_proc_hop_left(cell)
    case(hop_right)
        call run_proc_hop_right(cell)
    case(hop_up)
        call run_proc_hop_up(cell)
    case default
...
subroutine touchup_cell(cell)
    ...
    call add_proc(nli_exc_left_down(cell), site)
    call add_proc(nli_exc_left_up(cell), site)
    call add_proc(nli_exc_right_down(cell), site)
    call add_proc(nli_exc_right_up(cell), site)
    call add_proc(nli_hop_down(cell), site)
    call add_proc(nli_hop_left(cell), site)
    call add_proc(nli_hop_right(cell), site)
    call add_proc(nli_hop_up(cell), site)
...
!******
    ...
    if (.not. no_banner) then
        print *, "+------------------------------------------------------------+"
        print *, "|                                                            |"
        print *, "| This kMC Model 'MyFirstDisplacement' was written by        |"
        print *, "|                                                            |"
    ...
    do k = 0, system_size(3)-1
        do j = 0, system_size(2)-1
            do i = 0, system_size(1)-1
                ...
                select case(layer)
                case (default)
                    call replace_species((/i, j, k, default_a/), null_species, empty)
                end select
            end do
        end do
    end do
