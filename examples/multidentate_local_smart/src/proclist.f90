...
integer(kind=iint), parameter, public :: CC_right = 0
integer(kind=iint), parameter, public :: CC_right_1 = 1
integer(kind=iint), parameter, public :: CC_up = 2
integer(kind=iint), parameter, public :: CC_up_1 = 3
integer(kind=iint), parameter, public :: empty = 4
integer(kind=iint), public :: default_species = empty


! Process constants

integer(kind=iint), parameter, public :: CC_adsorption_right = 1
integer(kind=iint), parameter, public :: CC_adsorption_up = 2
integer(kind=iint), parameter, public :: CC_desorption_right = 3
integer(kind=iint), parameter, public :: CC_desorption_up = 4
...
!******
    ...
    select case(proc)
    case(CC_adsorption_right)
        call put_CC_right_simple_cubic_hollow(lsite)
        call put_CC_right_1_simple_cubic_hollow(lsite + (/1, 0, 0, 0/))

    case(CC_adsorption_up)
        call put_CC_up_simple_cubic_hollow(lsite)
        call put_CC_up_1_simple_cubic_hollow(lsite + (/0, 1, 0, 0/))

    case(CC_desorption_right)
        call take_CC_right_simple_cubic_hollow(lsite)
        call take_CC_right_1_simple_cubic_hollow(lsite + (/1, 0, 0, 0/))

    case(CC_desorption_up)
        call take_CC_up_simple_cubic_hollow(lsite)
        call take_CC_up_1_simple_cubic_hollow(lsite + (/0, 1, 0, 0/))

    end select

end subroutine run_proc_nr

subroutine put_CC_right_simple_cubic_hollow(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, empty, CC_right)

    ! disable affected processes
    if(avail_sites(CC_adsorption_right, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(CC_adsorption_right, site)
    endif

    if(avail_sites(CC_adsorption_right, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(CC_adsorption_right, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(CC_adsorption_up, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(CC_adsorption_up, site)
    endif

    if(avail_sites(CC_adsorption_up, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(CC_adsorption_up, site + (/0, -1, 0, 0/))
    endif

    ! enable affected processes
    select case(get_species(site + (/1, 0, 0, 0/)))
    case(CC_right_1)
        call add_proc(CC_desorption_right, site)
    end select


end subroutine put_CC_right_simple_cubic_hollow

subroutine take_CC_right_simple_cubic_hollow(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, CC_right, empty)

    ! disable affected processes
    if(avail_sites(CC_desorption_right, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(CC_desorption_right, site)
    endif

    ! enable affected processes
    select case(get_species(site + (/1, 0, 0, 0/)))
    case(empty)
        call add_proc(CC_adsorption_right, site)
    end select

    select case(get_species(site + (/-1, 0, 0, 0/)))
    case(empty)
        call add_proc(CC_adsorption_right, site + (/-1, 0, 0, 0/))
    end select

    select case(get_species(site + (/0, 1, 0, 0/)))
    case(empty)
        call add_proc(CC_adsorption_up, site)
    end select

    select case(get_species(site + (/0, -1, 0, 0/)))
    case(empty)
        call add_proc(CC_adsorption_up, site + (/0, -1, 0, 0/))
    end select


end subroutine take_CC_right_simple_cubic_hollow

subroutine put_CC_right_1_simple_cubic_hollow(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, empty, CC_right_1)
    ...
    select case(get_species(site + (/-1, 0, 0, 0/)))
    case(CC_right)
        call add_proc(CC_desorption_right, site + (/-1, 0, 0, 0/))
    end select


end subroutine put_CC_right_1_simple_cubic_hollow

subroutine take_CC_right_1_simple_cubic_hollow(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, CC_right_1, empty)

    ! disable affected processes
    if(avail_sites(CC_desorption_right, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(CC_desorption_right, site + (/-1, 0, 0, 0/))
    endif
...
end subroutine take_CC_right_1_simple_cubic_hollow

subroutine put_CC_up_simple_cubic_hollow(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, empty, CC_up)
    ...
    select case(get_species(site + (/0, 1, 0, 0/)))
    case(CC_up_1)
        call add_proc(CC_desorption_up, site)
    end select


end subroutine put_CC_up_simple_cubic_hollow

subroutine take_CC_up_simple_cubic_hollow(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, CC_up, empty)

    ! disable affected processes
    if(avail_sites(CC_desorption_up, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(CC_desorption_up, site)
    endif
...
end subroutine take_CC_up_simple_cubic_hollow

subroutine put_CC_up_1_simple_cubic_hollow(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, empty, CC_up_1)
    ...
    select case(get_species(site + (/0, -1, 0, 0/)))
    case(CC_up)
        call add_proc(CC_desorption_up, site + (/0, -1, 0, 0/))
    end select


end subroutine put_CC_up_1_simple_cubic_hollow

subroutine take_CC_up_1_simple_cubic_hollow(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, CC_up_1, empty)

    ! disable affected processes
    if(avail_sites(CC_desorption_up, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(CC_desorption_up, site + (/0, -1, 0, 0/))
    endif
...
end subroutine take_CC_up_1_simple_cubic_hollow

subroutine touchup_simple_cubic_hollow(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    if (can_do(CC_adsorption_right, site)) then
        call del_proc(CC_adsorption_right, site)
    endif
    if (can_do(CC_adsorption_up, site)) then
        call del_proc(CC_adsorption_up, site)
    endif
    if (can_do(CC_desorption_right, site)) then
        call del_proc(CC_desorption_right, site)
    endif
    if (can_do(CC_desorption_up, site)) then
        call del_proc(CC_desorption_up, site)
    endif
