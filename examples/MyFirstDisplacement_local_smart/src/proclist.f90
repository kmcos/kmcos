...
!******
    ...
    select case(proc)
    case(exc_left_down)
        call put_Au_default_a(lsite)
        call take_Au_default_a(lsite + (/1, 1, 0, 0/))
    ...
    case(exc_right_down)
        call put_Au_default_a(lsite + (/1, -1, 0, 0/))
        call take_Au_default_a(lsite)
    ...
    case(hop_down)
        call put_Au_default_a(lsite)
        call take_Au_default_a(lsite + (/0, 1, 0, 0/))

    case(hop_left)
        call put_Au_default_a(lsite)
        call take_Au_default_a(lsite + (/1, 0, 0, 0/))

    case(hop_right)
        call put_Au_default_a(lsite + (/1, 0, 0, 0/))
        call take_Au_default_a(lsite)

    case(hop_up)
        call put_Au_default_a(lsite + (/0, 1, 0, 0/))
        call take_Au_default_a(lsite)
...
subroutine put_Au_default_a(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, empty, Au)

    ! disable affected processes
    if(avail_sites(exc_left_down, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(exc_left_down, site)
    endif

    if(avail_sites(exc_left_up, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(exc_left_up, site)
    endif

    if(avail_sites(exc_right_down, lattice2nr(site(1) + (-1), site(2) + (1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(exc_right_down, site + (/-1, 1, 0, 0/))
    endif

    if(avail_sites(exc_right_up, lattice2nr(site(1) + (-1), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(exc_right_up, site + (/-1, -1, 0, 0/))
    endif

    if(avail_sites(hop_down, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(hop_down, site)
    endif

    if(avail_sites(hop_left, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(hop_left, site)
    endif

    if(avail_sites(hop_right, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(hop_right, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(hop_up, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(hop_up, site + (/0, -1, 0, 0/))
    endif

    ! enable affected processes
    select case(get_species(site + (/-1, -1, 0, 0/)))
    case(empty)
        call add_proc(exc_left_down, site + (/-1, -1, 0, 0/))
    end select

    select case(get_species(site + (/-1, 1, 0, 0/)))
    case(empty)
        call add_proc(exc_left_up, site + (/-1, 1, 0, 0/))
    end select

    select case(get_species(site + (/1, -1, 0, 0/)))
    case(empty)
        call add_proc(exc_right_down, site)
    end select

    select case(get_species(site + (/1, 1, 0, 0/)))
    case(empty)
        call add_proc(exc_right_up, site)
    end select

    select case(get_species(site + (/0, -1, 0, 0/)))
    case(empty)
        call add_proc(hop_down, site + (/0, -1, 0, 0/))
    end select

    select case(get_species(site + (/-1, 0, 0, 0/)))
    case(empty)
        call add_proc(hop_left, site + (/-1, 0, 0, 0/))
    end select

    select case(get_species(site + (/1, 0, 0, 0/)))
    case(empty)
        call add_proc(hop_right, site)
    end select

    select case(get_species(site + (/0, 1, 0, 0/)))
    case(empty)
        call add_proc(hop_up, site)
    end select


end subroutine put_Au_default_a

subroutine take_Au_default_a(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    ! update lattice
    call replace_species(site, Au, empty)

    ! disable affected processes
    if(avail_sites(exc_left_down, lattice2nr(site(1) + (-1), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(exc_left_down, site + (/-1, -1, 0, 0/))
    endif

    if(avail_sites(exc_left_up, lattice2nr(site(1) + (-1), site(2) + (1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(exc_left_up, site + (/-1, 1, 0, 0/))
    endif

    if(avail_sites(exc_right_down, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(exc_right_down, site)
    endif

    if(avail_sites(exc_right_up, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(exc_right_up, site)
    endif

    if(avail_sites(hop_down, lattice2nr(site(1) + (0), site(2) + (-1), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(hop_down, site + (/0, -1, 0, 0/))
    endif

    if(avail_sites(hop_left, lattice2nr(site(1) + (-1), site(2) + (0), site(3) + (0), site(4) + (0)), 2).ne.0)then
        call del_proc(hop_left, site + (/-1, 0, 0, 0/))
    endif

    if(avail_sites(hop_right, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(hop_right, site)
    endif

    if(avail_sites(hop_up, lattice2nr(site(1), site(2), site(3), site(4)), 2).ne.0)then
        call del_proc(hop_up, site)
    endif

    ! enable affected processes
    select case(get_species(site + (/1, 1, 0, 0/)))
    case(Au)
        call add_proc(exc_left_down, site)
    end select

    select case(get_species(site + (/1, -1, 0, 0/)))
    case(Au)
        call add_proc(exc_left_up, site)
    end select

    select case(get_species(site + (/-1, 1, 0, 0/)))
    case(Au)
        call add_proc(exc_right_down, site + (/-1, 1, 0, 0/))
    end select

    select case(get_species(site + (/-1, -1, 0, 0/)))
    case(Au)
        call add_proc(exc_right_up, site + (/-1, -1, 0, 0/))
    end select

    select case(get_species(site + (/0, 1, 0, 0/)))
    case(Au)
        call add_proc(hop_down, site)
    end select

    select case(get_species(site + (/1, 0, 0, 0/)))
    case(Au)
        call add_proc(hop_left, site)
    end select

    select case(get_species(site + (/-1, 0, 0, 0/)))
    case(Au)
        call add_proc(hop_right, site + (/-1, 0, 0, 0/))
    end select

    select case(get_species(site + (/0, -1, 0, 0/)))
    case(Au)
        call add_proc(hop_up, site + (/0, -1, 0, 0/))
    end select


end subroutine take_Au_default_a

subroutine touchup_default_a(site)

    integer(kind=iint), dimension(4), intent(in) :: site

    if (can_do(exc_left_down, site)) then
        call del_proc(exc_left_down, site)
    endif
    if (can_do(exc_left_up, site)) then
        call del_proc(exc_left_up, site)
    endif
    if (can_do(exc_right_down, site)) then
        call del_proc(exc_right_down, site)
    endif
    if (can_do(exc_right_up, site)) then
        call del_proc(exc_right_up, site)
    endif
    if (can_do(hop_down, site)) then
        call del_proc(hop_down, site)
    endif
    if (can_do(hop_left, site)) then
        call del_proc(hop_left, site)
    endif
    if (can_do(hop_right, site)) then
        call del_proc(hop_right, site)
    endif
    if (can_do(hop_up, site)) then
        call del_proc(hop_up, site)
    endif
